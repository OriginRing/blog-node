...
export const markedFunction = (path: string) => {
    ...
    articleList.map(item => {
        ...
        const json = {id, name, author, introduce, date, content: str}
        writeFileSync(`${process.cwd()}/json/${id}.json`, JSON.stringify(json));
        list.push({id, name, author, date, introduce})
    })
    list.sort(function(a, b) {
        return new Date(b.date).getTime() || 0 < new Date(a.date).getTime() || 0 ? -1 : 1
    })
